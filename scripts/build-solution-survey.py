...
⚠ 이건 **조사 수집용**이고 시스템에 자동으로 들어가지 않는다. 채워서 모으면
  ISV BU 가 /admin 폼에 옮겨 적는다(사용자 결정). JSON 을 조사자에게 쓰게 하지
...
BASIC = [
    ...
    ('4-Layer 분류 *',   '[ISV BU 분류] 아키텍처상 어느 층인가 — 모르면 비워두세요',                                        30, options('sol-layer')),
    ('제공 형태',        '예) SaaS/API · SW(On-prem) · Hybrid',                           18, None),
    ('MZC 시너지',       '[ISV BU 분류] MZC 오퍼링·MSP 와 얼마나 맞물리나 — 모르면 비워두세요',                             12, options('sol-synergy')),
    ...
    ('서비스 급',        '[ISV BU 분류] 모르면 비워두세요',                                            10, options('sol-grade')),
    ('규모급',           '[ISV BU 분류] 모르면 비워두세요',                                            12, options('sol-scale')),
    ('포컬 담당',        '[ISV BU 분류] MZC 내부 담당자. 모르면 비워두세요',                       14, None),
    ('운영 상태',        '[ISV BU 분류] 지금 팔 수 있는 상태인가 — 모르면 비워두세요',                                      18, options('sol-status-op')),
...
BASIC += [('MZC 내부 의견', '[ISV BU 분류] 전략·마진 코멘트. ⚠ 고객에게 노출되지 않습니다', 34, None)]

SECTIONS = [
    ('§1 솔루션 개요 / 핵심 차별점', '제공 형태·카테고리·JTBD 와 **차별적 비즈니스 가치**. '
     '여기 「① 라벨: 설명」 형식으로 쓴 것이 세일즈 피치의 강점 3줄로 그대로 나갑니다'),
    ('§1 내부 코멘트',              '[ISV BU 분류] 🔒 고객 미노출. AI Tech 의견 등'),
    ...
    ('§8 내부 코멘트',              '[ISV BU 분류] 🔒 고객 미노출. 마진·딜사이즈 전략'),
]

JUDGE = [
    ('역할 슬롯 *',   '[ISV BU 분류] 「선택지」 시트의 슬롯 ID. **없으면 추천 후보가 되지 않습니다** — 모르면 비워두세요', 22, None),
    ('번들 확장성',   '[ISV BU 분류] SI·MSP 와 얼마나 묶이나 — 모르면 비워두세요',                                                    24, options('sol-bundle-potential')),
]
JUDGE += [(f'{aid} {name}', f'주요 우려: {concern}', 13, ['0', '1', '2', '3']) for aid, name, concern in AREAS]
JUDGE += [
...
SIMSHEET = [
    ('시뮬레이터 해당 옵션', '[ISV BU 분류] 「선택지」 시트의 옵션 ID 를 쉼표로. 예) q1_3, q4_2 — 모르면 비워두세요', 60, None),
]
...
def build(out_path, rows):
    guide = [
        ...
        ['④ JSON 안 씁니다', '전제 조건·부적합 신호·구간 단가는 **평문으로** 적어주세요. '
                          'ISV BU 가 시스템 형식으로 옮깁니다. 형식을 맞추려다 틀리는 것보다 낫습니다.'],
        ['⑤ 내부 전용', '「MZC 내부 의견」과 「§1·§8 내부 코멘트」는 고객에게 노출되지 않습니다. '
                          '마진·전략 이야기는 반드시 이 칸에만 쓰세요 — 본문에 쓰면 고객 문서로 나갑니다.'],
        ['⑥ [ISV BU 분류] 표시', '설명에 [ISV BU 분류] 가 붙은 칸은 **우리 내부 분류 체계**입니다 '
                          '(4-Layer·역할 슬롯·급·시뮬레이터 매핑 등). 제품을 잘 아셔도 이 값은 모르는 게 정상이니 '
                          '비워두시면 ISV BU 가 채웁니다. 나머지 칸이 조사해 주실 부분입니다.'],
        ['⑦ 평가영역 강도', '「3_판정데이터」의 A01~A10 은 **이 제품이 해결해 주는 영역**입니다. '
                          '아는 만큼 0~3 으로 적어주시면 ISV BU 가 검토합니다. 열 설명에 각 영역의 주요 우려가 적혀 있습니다.'],
        ['⑧ 선택지', '드롭다운이 없는 칸(역할 슬롯·시뮬레이터 옵션)은 「선택지」 시트에서 ID 를 찾아 적으세요.'],
        ['⑨ 다 채우면', '파일 그대로 ISV BU 에 보내주세요. 시스템 등록은 ISV BU 가 합니다.'],
    ]
    picks = ([('── 역할 슬롯 (3_판정데이터)', '', '')]
