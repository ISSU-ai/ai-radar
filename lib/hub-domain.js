...
const { normaliseHandoff } = require('./handoff-fields');
...
const PIPELINE_STAGES = Object.freeze([
  ...
  '딜 사이즈',
  '피치 준비',
  // 051. 배포 인계는 피치 다음 단계다. 단계로 둬야 041 의 정체 시계와 목록 필터가
  // 그대로 작동한다 — 별도 화면으로 빼면 「인계 대기 3주」를 아무도 못 본다.
  '배포 인계'
]);
...
const EDITABLE_DEAL_FIELDS = Object.freeze([
  ...
  // 배포 인계(051). 값과 근거(050 인용 앵커)를 같이 담는 jsonb 한 칸이다.
  'handoff',
  'track',
  'isv_combo',
  'packages',
...
function clampStage(value) {
  ...
  if (!Number.isInteger(parsed) || parsed < 0 || parsed >= PIPELINE_STAGES.length) {
    throw new Error(`단계는 0부터 ${PIPELINE_STAGES.length - 1} 사이의 정수여야 합니다.`);
  }
  return parsed;
}
...
function normaliseDealPatch(input) {
  ...
  for (const field of EDITABLE_DEAL_FIELDS) {
    ...
    // 051. jsonb 한 칸이라 아무거나 들어올 수 있다. 모르는 키는 버린다 —
    // 한 번 들어가면 다음에 읽는 코드가 그걸 진짜인 줄 안다.
    else if (field === 'handoff') patch.handoff = normaliseHandoff(ensurePlainObject(body.handoff, field)); else if (field === 'isv_combo' || field === 'packages') {
