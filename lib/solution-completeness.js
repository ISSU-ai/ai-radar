 ...
 * viewer 에게 노출되고 있었다. ISSU 에게 등록 권한을 열면서 게이트를 세우지 않으면
 * 같은 사고가 반복된다.
 *
...
function evaluateCompleteness(solution, context = {}) {
  ...
  const coverage = asArray(solution.fqa_coverage);
  const prerequisites = asArray(solution.prerequisites);
  ...
  if (coverage.length === 0) {
    add(blocking, 'coverage_missing',
      '메우는 준비도 갭(fqa_coverage)이 비어 있습니다. 이대로는 어떤 고객에게도 추천되지 않습니다.');
  }
