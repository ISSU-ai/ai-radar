...
const HANDOFF_FIELDS = Object.freeze([
  Object.freeze({
    ...
    // Kit §1 이 「지금 추진해야 하는 이유가 일정·의사결정·마감일과 연결되어 있는가」를
    // 체크 항목으로 둔다. 영업이 미팅에서 듣는 것이고, 없으면 배포팀이 급한 이유를 모른다.
    ask: '지금 추진해야 하는 이유가 무엇입니까 — 어떤 일정·의사결정·마감일과 연결됩니까?'
  }),
  Object.freeze({
...
/**
 * Kit §7 「확인된 제약사항」 10체크. **영업이 미팅에서 확인하는 것들이다** —
 * 예산 승인이 났는지, 보안 검토가 걸려 있는지는 영업이 안다.
 *
 * 체크박스라 부담이 작다. 제약은 **있다/없다**이지 설명이 필요한 게 아니다.
 * 비어 있으면 배포팀이 열 개를 다시 묻는다.
 */
const CONSTRAINTS = Object.freeze([
  Object.freeze({ key: 'budget', label: '예산 또는 상업적 승인' }),
  Object.freeze({ key: 'timeline', label: '일정 또는 출시 마감일' }),
  Object.freeze({ key: 'security', label: '보안 · 컴플라이언스 검토' }),
  Object.freeze({ key: 'residency', label: '데이터 레지던시 · 리전 간 처리 제한' }),
  Object.freeze({ key: 'workspace', label: '워크스페이스 구성 · 관리자 대응 여력' }),
  Object.freeze({ key: 'connector', label: '커넥터 · 시스템 연동' }),
  Object.freeze({ key: 'license', label: '라이선스 또는 좌석 확보' }),
  Object.freeze({ key: 'dataQuality', label: '데이터 품질 · 지식 소스 준비도' }),
  Object.freeze({ key: 'userReadiness', label: '사용자 준비도 · 변화관리' }),
  Object.freeze({ key: 'language', label: '언어 · 현지화 · 지원 범위' })
]);
const CONSTRAINT_KEYS = Object.freeze(CONSTRAINTS.map((c) => c.key));

/**
 * Kit §7 리스크. 원본 표는 7컬럼 × 6행이지만 **설명과 영향도 둘만** 받는다.
 *
 * 42칸을 받으면 아무도 안 채우고, 안 채워진 표는 없는 것과 같다. 그리고 나머지 넷
 * (책임자·완화방안·발생가능성·상태)은 **영업이 정할 수 있는 것이 아니다** —
 * 배포 단계에서 붙는다.
 */
const RISK_LIMIT = 6;
const RISK_IMPACTS = Object.freeze([['high', '높음'], ['mid', '보통'], ['low', '낮음']]);
const impactLabel = (value) => (RISK_IMPACTS.find(([key]) => key === value) || [])[1] || '';
...
function normaliseHandoff(input) {
  ...
  // Kit §7 제약 — 체크된 것만. 모르는 키는 버린다.
  const constraints = asArray(source.constraints).filter((key) => CONSTRAINT_KEYS.includes(key));
  if (constraints.length) out.constraints = constraints;

  // Kit §7 리스크 — 설명이 없으면 행을 안 만든다. 빈 행은 「없다」가 아니라 소음이다.
  const risks = asArray(source.risks)
    .map((row) => {
      const entry = asObject(row);
      return {
        text: text(entry.text).trim().slice(0, 500),
        impact: RISK_IMPACTS.some(([key]) => key === entry.impact) ? entry.impact : ''
      };
    })
    .filter((row) => row.text)
    .slice(0, RISK_LIMIT);
...
const api = {
  ...
  CONSTRAINTS, CONSTRAINT_KEYS, RISK_LIMIT, RISK_IMPACTS, impactLabel,
