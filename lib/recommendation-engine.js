 ...
 * 갭은 같은 무게로 다룰 수 없다.
 */
function analyseGaps(fqaTotals, itemCountByCategory) {
  const gaps = {};
 ...
 */
function evaluatePrerequisites(candidate, deal, gaps, itemScores) {
  const blockedBy = [];
  const pendingManual = [];
  const enablers = [];
  const confirmations = asObject(deal.prereq_confirmations)[candidate.slug] || {};
  ...
  for (const prereq of asArray(candidate.prerequisites)) {
    ...
    if (prereq.kind === 'fqa') {
      // 문항 점수가 있으면 그것을, 없으면 카테고리 평균을 본다.
      const itemScore = itemScores?.[prereq.item];
      const categoryScore = gaps[prereq.category]?.score;
      const actual = Number.isFinite(itemScore) ? itemScore : categoryScore;
      if (!Number.isFinite(actual)) continue; // 모르면 거르지 않는다
      if (actual < Number(prereq.min)) {
        const entry = {
          label: prereq.label || `${prereq.category}[${prereq.item}] ${prereq.min} 이상`,
          category: prereq.category, item: prereq.item,
...
/**
 * @param {object} input
 * @param {object} input.deal              fqa_totals · customer_meta · track · prereq_confirmations
 ...
 * @param {object} input.itemScores        문항명 → 점수 (있으면 문항 단위 전제 판정)
...
function recommend(input = {}) {
  ...
  const gaps = analyseGaps(deal.fqa_totals, input.itemCountByCategory);
  ...
  const evaluateCandidate = (candidate, kind) => {
    ...
    // T-D = 타사 SWG 검토형. 보안 게이트웨이 슬롯을 새로 넣으면 기존 SWG 와 충돌한다.
    if (filters.security_stack && deal.track === 'T-D' && candidate.slot === 'security-gateway') {
      excludedBy.push('타사 SWG 운영 중(T-D) — 게이트웨이 중복');
    }
    ...
    const prereq = kind === 'solution'
      ? evaluatePrerequisites(candidate, deal, gaps, input.itemScores)
    ...
    return {
      ...
      readinessLift: asObject(candidate.readiness_lift),
   ...
   * 패키지의 fqa_coverage 와 전제의 카테고리·문항을 대조하면 자동으로 나온다.
  ...
  const findImplicitEnabler = (blockedBy) => {
    for (const need of blockedBy) {
      ...
      const byLift = eligible.find((cand) => {
        ...
        return projected >= Number(need.required);
      });
      if (byLift) return byLift;

      const match = eligible.find((cand) => asArray(cand.rawCoverage).some((entry) => {
        if (entry?.category !== need.category) return false;
        if ((Number(entry.strength) || 0) < 2) return false;
        const items = asArray(entry.items);
        return !need.item || items.length === 0 || items.includes(need.item);
      }));
  ...
  function bundleEnablerReason(item, enabler) {
    const need = item.prerequisites.blockedBy.find((b) => b.category && enabler.readinessLift?.[b.category]);
    if (need) {
      const lift = Number(enabler.readinessLift[need.category]);
      const projected = Number(((Number(need.actual) || 0) + lift).toFixed(1));
      return `→ ${enabler.name} 선행 시 ${need.category} ${need.actual} → ${projected} 예상`
        + ` (전제 ${need.required} 충족)`;
    }
  ...
  return {
    ...
    reviewed: Boolean(deal.fqa_reviewed_at),
    label: deal.fqa_reviewed_at ? '실사 반영 추천' : '고객 자가응답 기준 잠정 추천',
