...
function recommend(input = {}) {
   ...
   */
  const coversNeed = (cand, need) => asArray(cand.rawCoverage).some((entry) => {
  ...
  const findImplicitEnabler = (blockedBy) => {
    for (const need of blockedBy) {
      ...
      const byLift = eligible.find((cand) => liftFor(cand, need));
      if (byLift) return byLift;

      const match = eligible.find((cand) => coversNeed(cand, need));
