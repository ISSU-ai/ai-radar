...
function evaluatePrerequisites(candidate, deal, gaps, itemScores) {
  ...
  for (const prereq of asArray(candidate.prerequisites)) {
    ...
    if (prereq.kind === 'fqa') {
      // 전제가 특정 문항을 지목했으면 **그 문항의 점수만** 본다.
      //
      // 예전에는 없을 때 카테고리 평균으로 때웠다. 고객이 21문항을 전부 답하던
      // 시절에는 드문 경로라 문제가 안 됐지만, 지금은 42문항이 030 bridge 로
      // 채우는 13개뿐이라 A 영역은 6문항 중 2개만 찬다. 다른 두 문항의 평균으로
      // "접근권한이 3점 이상인가" 를 판정하면 그게 조용히 틀리는 자리다.
      //
      // 모르면 영업에게 물어본다. 판정을 지어내지 않는다.
      const itemScore = itemScores?.[prereq.item];
      const actual = prereq.item
        ? (Number.isFinite(itemScore) ? itemScore : null)
        : gaps[prereq.category]?.score;
      if (!Number.isFinite(actual)) {
        const label = prereq.label || `${prereq.category}[${prereq.item}] ${prereq.min} 이상`;
        if (confirmations[label] !== true && prereq.blocking !== false) {
          pendingManual.push({ label, kind: 'fqa', item: prereq.item, min: Number(prereq.min) });
        }
        continue;
      }
...
function recommend(input = {}) {
  ...
  const strengthFor = (cand, need) => {
    let best = 0;
    for (const entry of asArray(cand.rawCoverage)) {
