...
function recommend(input = {}) {
  ...
  const evaluateCandidate = (candidate, kind) => {
    ...
    // 고객 폼은 제품명을 묻지 않는다 — 보안 담당자가 아닌 사람도 답해야 한다.
    // existing·managed 는 "장비는 있는데 무엇인지는 모른다" 라서 **판정하지 않는다.**
    // 모르는 것으로 후보를 거르면 조용히 틀린다. 영업이 STEP01 에서 제품을 확인해
    // other-swg 로 바꾸면 그때 걸린다.
    const swg = String(meta.securityStack || meta.security || '').toLowerCase();
    const unknownStack = ['none', '없음', 'unknown', '미정', 'existing', 'managed'];
    const otherSwg = swg && !swg.includes('zscaler') && !unknownStack.includes(swg);
   ...
   * enabled_by 를 일일이 적지 않아도 되므로 ISV BU 입력 부담도 줄어든다.
   */
