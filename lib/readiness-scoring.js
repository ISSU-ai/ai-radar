 ...
 * @param {boolean} [options.partial]  미응답을 허용하고 답한 것만으로 낸다.
 *   고객 진단(/readiness)은 false — 부분 응답으로 점수를 내면 "3점을 안 골랐는데
 *   3점으로 계산된" 결과가 나온다. 영업이 허브에서 채워 넣는 중일 때만 true 다.
 */
function scoreReadiness(items, areas, rawScores, options = {}) {
  ...
  const answeredIn = (list) => list.filter((item) => {
    const raw = scores[item.code];
  ...
  const areaScores = [...byArea.entries()]
    .map(([id, list]) => {
      // partial 에서는 답한 문항만으로 낸다. 미응답을 0 이나 3 으로 치면 영업이
      // 절반 채운 딜의 축 점수가 실제보다 낮거나 높게 나온다.
      const answeredItems = answeredIn(list);
      const sum = answeredItems.reduce((acc, item) => acc + Number(scores[item.code]), 0);
      return {
        area: id,
        name: areaName.get(id) || id,
        score: answeredItems.length ? round2(sum / answeredItems.length) : null,
  ...
  const scored = areaScores.filter((a) => a.score !== null);
  const answeredCount = items.length - unanswered.length;

  // 답한 게 없으면 점수라고 부를 것이 없다. 0 이나 3 을 지어내지 않는다.
  if (!scored.length) {
    return {
      average: null, maturity: null, areas: areaScores,
      weakest: null, strongest: null, lowAreas: [], priorities: [],
      insight: '아직 응답이 없습니다.',
      answers: [], answeredCount: 0, totalCount: items.length, partial: true
    };
  }

  // 종합은 문항 평균이 아니라 **축 평균의 평균**이다. 축마다 문항 수가 같아
  // 지금은 결과가 같지만, 문항이 늘어도 축 하나가 종합을 끌고 가지 않는다.
  const average = round2(scored.reduce((acc, a) => acc + a.score, 0) / scored.length);
  ...
  const answers = items
    ...
    .map((item) => {
      ...
      return {
        ...
        // 「그래서 무엇을 하면 되는가」(043). 목표에 이미 닿았으면 붙이지 않는다 —
        // 잘하고 있는 항목에 처방을 달면 나머지 처방도 같이 안 읽힌다.
        fix: (Math.max(0, Number(item.target ?? 4) - value) > 0 && item.fix) ? item.fix : ''
      };
    });
  ...
  return {
    ...
    priorities: ranked.slice(0, 3).map((a) => ({
      ...
      items: answers
        ...
        .slice(0, 3)
        .map(({ code, text, score, rubric, gap, fix }) => ({ code, text, score, rubric, gap, fix }))
    })),
    ...
    partial: answeredCount < items.length
  };
}
