...
const { HANDOFF_FIELDS, QUALITY_CHECKS, qualityLabel, handoffReadiness,
  interviewQuestions, qualityGaps } = req ? req('./handoff-fields') : global.IssuHandoffFields;
...
function briefEntry(section, ctx) {
  ...
  switch (section.from) {
    case 'problem': {
      const lines = [deal.lead_message, valueOf(handoff, 'workflow')].filter(Boolean);
    ...
    case 'assumptions':
    case 'risks': {
      const lines = asArray(openItems);
      const gaps = qualityGaps(handoff).map((g) => `${g.label} — ${g.levelLabel}`);
      const all = section.from === 'risks' ? lines.concat(gaps) : lines;
      return { value: all.length ? bullet(all) : '', status: all.length ? OPEN : OPEN,
        source: all.length ? '시스템이 모으는 미확인 항목' : '' };
    }
    case 'recommendation':
...
const api = {
  ...
  buildBrief, buildInterviewGuide, buildEvidenceSummary, buildHandoffExport,
  HANDOFF_FIELDS
};
