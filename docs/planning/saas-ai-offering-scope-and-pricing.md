...
| 항목 | 값 | 성격 |
|---|---|---|
| **환율** | **1 USD = 1,400원** | 고정 가정 |
| Blended Monthly ARPU | 고객 규모별 **$38 · $46 · $54** | 내부 가정 (Seat·Credit·API 사용 가능성 반영) |
| Codex 사용금액 | 개발자당 월 $100~200 | **내부 사업성 검토용 참고 범위 — 고객 예산 보장값 아님** |
| ChatGPT Enterprise | — | **확정 사실로 기재하지 않고 "OpenAI 협의사항" 으로 표시** |

**목표 매출 시뮬레이션**

| 구분 | 고객 수 | 고객 구성 | 예상 시트 | 연 매출 |
|---|:---:|---|:---:|---|
| Commit Case | 100개사 | SMB 70 / DNB 25 / Ent 5 | 6,100석 | 약 46.33억원 |
| Stretch Case | 500개사 | SMB 350 / DNB 125 / Ent 25 | 30,500석 | 약 231.67억원 |

> 연 매출 = Σ(고객군별 총 시트 × Blended 월 ARPU) × 12개월 × 1,400원/USD
> **AIR PS·ISV·MS 매출과 마진은 별도 원가·공수 기준으로 산정한다.**
...
| 항목 | 값 | 위치 |
|---|---|---|
| **전역 환율** | 1 USD = **1,400원** | admin 전역 설정 (현재 값 확인 필요) |
